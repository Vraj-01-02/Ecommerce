 ...
 const[visible,setVisible] = useState(false);
  ...
  return (
    <div className='flex items-center justify-between py-5 font-medium'>
        ...
        <div className={`absolute top-0 right-0 bottom-0 w-full bg-white transition-all ${visible ? 'w-full' : 'w-0'}`}>
          <div onClick={()=>setVisible(false)} className='flex items-center gap-4 p-3'>
            <img className='h-4 rotate-180' src={assets.dropdown_icon} alt="" />
            <p>Back</p>
          </div>
          <ul className='flex flex-col gap-2 pl-6 text-gray-600'>
            <NavLink to='/' className='py-2'>HOME</NavLink>
            <NavLink to='/collection' className='py-2'>COLLECTION</NavLink>
